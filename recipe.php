...
if (!$recipeNotFound) {
    ...
    if ($recipe_id) {
        $numSimilar = rand(3, 7);
        $similarRecipes = $recipeController->getSimilarRecipesByEmbedding($recipe_id, $numSimilar); // Get up to 5 similar recipes
    }
}
			...
			<div class="bg-white rounded-lg shadow-md p-6">
				...
				<div class="flex justify-between items-start mb-4">
					...
					<div class="flex space-x-4">
						...
						<!-- Share Icon -->
						<a href="social.php?recipe_id=<?= $recipe['id'] ?>&recipe_name=<?= urlencode($recipe['recipe']) ?>"
